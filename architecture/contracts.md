...
В release 03 оба эндпоинта являются заглушками и возвращают `NOT_FOUND`.

`rest-core` не вызывает `ai-core` и не обращается к `ProxyAI`. Интеграция с AI будет подключена в следующих релизах.
